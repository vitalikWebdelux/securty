<div class="bc-slide step">
	...
	<?php for($i = 1; $i <= 72; $i++){ ?>
		<?php if($i <= 8){ 
			$name = 'STANDART';
			$fromPrice = '299';
			$item1 =  __('Центральний блок', 'sequrity').'<span>1шт</span>';
			$item2 =  __('Датчик руху', 'sequrity').'<span>1шт</span>';
			$item3 =  __('Датчик відкриття дверей', 'sequrity').'<span>1шт</span>';
			$item4 =  __('Тривожна кнопка', 'sequrity').'<span>1шт</span>';
				...
				<div class="taryf__linkr">
					<label class="taryf__overlay" for="taryf-<?php echo $i; ?>">
						...
						<h4 class="taryf__header">
							...
							<?php if($name == "ABONPLATA" || $name == "ABONPLATA+"){ ?>
								<p class="taryf__tooltip">
									<span>Оборудование передается в аренду. Аренда включена в стоимость абонплаты</span>
								</p>
							<?php } ?>
						</h4>
						...
						<div class="taryf__body">
							...
							<ul>
								<li><span><?php echo $item1; ?></span><span class="taryf__dashed"></span></li>
								...
								<li><span><?php echo $item4; ?></span><span class="taryf__dashed"></span></li>
							</ul>
						</div>
